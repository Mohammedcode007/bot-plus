...
} from '../services/fishGame.service.js';

const fishCooldownMap = new Map();

function parseFishCommand(text) {
...
async function checkCooldown(roomMessage) {
  ...
  const cooldownMs = (await getFishCooldownSeconds()) * 1000;

  const now = Date.now();
  const lastTime = fishCooldownMap.get(key) || 0;
...
function cooldownText({
  ...
  if (lang === 'ar') {
    return `⏱️ ${playerName} انتظر ${formatDuration(remainingMs, 'ar')} قبل الصيد مرة أخرى.`;
  }

  return `⏱️ ${playerName}, wait ${formatDuration(remainingMs, 'en')} before fishing again.`;
}
...
export async function handleFishCommand({
  ...
  const parsed = parseFishCommand(roomMessage.text);
  ...
  const result = await rollFishResult(currentPoints);

  if (!result.ok) {
    ws.sendRoomMessage(
      targetRoomId,
      lang === 'ar'
        ? `❌ فشل الصيد. السبب: ${result.message || result.type}`
        : `❌ Fishing failed. Reason: ${result.message || result.type}`,
      targetRoomName,
    );
  ...
  ws.sendRoomMessage(
    targetRoomId,
    [
      `🎣 ${playerName} went fishing!`,
      `you caught ${result.prize}`,
