...
function parseMusicCommand(text) {
  ...
  if (raw.toLowerCase().startsWith('play ')) {
    return {
      isCommand: true,
      type: 'current',
  ...
  if (
    raw.toLowerCase().startsWith('.ps ') ||
    raw.toLowerCase().startsWith('.so ') ||
    raw.toLowerCase().startsWith('.sh ')
  ) {
    return {
      isCommand: true,
      type: 'broadcast',
  ...
  if (command === '.ps' || command === '.so' || command === '.sh') {
    return {
      isCommand: true,
      type: 'broadcast',
      query: '',
...
function sendRoomAudioSafe(socket, roomId, roomName, url) {
  ...
  */
  if (typeof socket.sendRoomAudioUrl === 'function') {
    ...
    /*
      احتياطي فقط لو نسخة WsClient عندك تدعم:
  ...
  if (socket.ws && typeof socket.ws.sendRoomAudioUrl === 'function') {
    try {
      socket.ws.sendRoomAudioUrl(url, roomName || roomId);
      return true;
    } catch {}
    ...
    try {
      socket.ws.sendRoomAudioUrl(roomId, url, roomName);
      return true;
    } catch {}
  }
...
export async function handleControllerMusicCommand({
  roomMessage,
  ws,
  runtime,
  targetRoomId,
...
function sendRoomAudioSafe(socket, roomId, roomName, url) {
  ...
  if (socket.socket && typeof socket.socket.sendRoomAudioUrl === 'function') {
    socket.socket.sendRoomAudioUrl(
      roomId,
      url,
      roomName,
    );
  ...
  if (socket.client && typeof socket.client.sendRoomAudioUrl === 'function') {
    socket.client.sendRoomAudioUrl(
      roomId,
      url,
      roomName,
    );
